...
async function main() {
  ...
  const confirmateurPasswordHash = await bcrypt.hash('confirmateur123', 10);

  const admin = await prisma.user.upsert({
    where: { username: 'amine_admin' },
    update: { password: adminPasswordHash, role: Role.ADMIN },
    create: {
  ...
  const confirmateur = await prisma.user.upsert({
    where: { username: 'confirmateur_dz' },
    update: { password: confirmateurPasswordHash, role: Role.CONFIRMATEUR },
    create: {
      username: 'confirmateur_dz',
      password: confirmateurPasswordHash,
      role: Role.CONFIRMATEUR,
    },
  });

  console.log('Seeded Users:', admin.username, confirmateur.username);
  ...
  // 3. Seed Sample Orders matching HTML tables
  ...
  const sampleOrders = [
    ...
    },
    {
      ...
      productId: createdProducts[5]?.id || p2.id,
      priceTotal: 52000,
